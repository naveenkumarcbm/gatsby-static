import React from 'react'
import Layout from '../../../components/layout'

const SupplyChain = () => {
    return (<Layout headerclassName="white-bg-header">
  <div className="linear-gradient-1">
      <div className="aspot-bg-1 bg-cover bg-cover-bottom">
        <div className="content-container d-flex align-items-center">
          <div className="container">
            <div className="row justify-content-center">
              <div className="col-md-6 text-center">
                <p style={{textAlign: "center"}}>WHO WE HELP</p>
                <h1 className="heading-underline underline-blue" style={{textAlign: "center"}}>Supply Chain</h1>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="content-container">
        <div className="container">
          <div className="row">
            <div className="col-md-6 content-section">
              <h2 className="h3">CoEnterprise is helping companies just like yours identify, prioritize, and respond to supply chain execution problems every day.</h2>
            </div>
            <div className="col-md">
              <p>
                At CoEnterprise, we understand the supply chain marketplace. We’ve spent many years empowering our clients to operate more efficiently and drive faster, more efficient decision-making
                based on their data.
              </p>
              <p>
                <span style={{ fontWeight: 400}}
                  >As the leading provider of EDI and supply chain visibility software – <a href="/solutions/syncrofy">Syncrofy for EDI</a> – which is widely used by many
                  top fortune 100 companies, and an IBM Platinum Business Partner and one of IBM’s largest B2B integration software resellers, we know a thing or two.&nbsp;</span
                >
              </p>
              <p>
                <span style={{ fontWeight: 400}}
                  ><a href="/solutions/syncrofy_supply_chain/">Syncrofy for Supply Chain</a> (SSC) provides real-time visibility to break down barriers across the supply
                  chain. It’s advanced real-time analytics and insights help companies solve costly problems before they occur and realize revenue fast by eliminating costly fines and chargebacks,
                  late shipments, and late payments.</span
                >
              </p>
              <p>
                <a className="btn btn-orange-outline" href="https://info.coenterprise.com/hubfs/COE_ASN%20Compliance%201%20pager.pdf" target="_blank" rel="noopener"
                  >Learn About Our ASN Compliance Program</a
                >
              </p>
              <p>&nbsp;</p>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div className="content-container bg-full-width bg-shapes-4">
      <div className="container">
        <div className="row justify-content-center">
          <div className="col-md-6 content-section">
            <h2 className="heading-underline underline-purple" style={{textAlign: "center"}}>How Can We Help You?</h2>
          </div>
        </div>

        <div className="row justify-content-center">
          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/syncrofy_supply_chain/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="wp-image-649 size-thumbnail aligncenter"
                    src="../../SupplyChain_2_Visibility-300x300.png"
                    alt="Supply Chain Visibility"
                    width="75"
                    height="75"
                    srcSet="../../SupplyChain_2_Visibility-300x300.png 300w, ../../SupplyChain_2_Visibility.png 800w"
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I need full order lifecycle visibility in one place, from purchase order through to invoice</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/syncrofy_supply_chain/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-1049 size-thumbnail"
                    src="../../SupplyChain_3_OrderManagement-300x300.png"
                    alt="Order Management"
                    width="75"
                    height="75"
                    srcSet="
                      ../../SupplyChain_3_OrderManagement-300x300.png 300w,
                      ../../SupplyChain_3_OrderManagement.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I need help with order and exception management</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/syncrofy_supply_chain/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-1050 size-thumbnail"
                    src="../../SupplyChain_4_TrackKPIs-300x300.png"
                    alt="Track KPIs"
                    width="75"
                    height="75"
                    srcSet="../../SupplyChain_4_TrackKPIs-300x300.png 300w, ../../SupplyChain_4_TrackKPIs.png 800w"
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I need to calculate fill rate and lead time for both orders and shipments, in total and for each line item</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>
        </div>
        <div className="row justify-content-center">
          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/syncrofy_supply_chain/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="wp-image-1051 size-thumbnail aligncenter"
                    src="../../SupplyChain_5_OrderVisibility-300x300.png"
                    alt="Order Visibility"
                    width="75"
                    height="75"
                    srcSet="
                      ../../SupplyChain_5_OrderVisibility-300x300.png 300w,
                      ../../SupplyChain_5_OrderVisibility.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I need to reconcile invoices against purchase order, shipment documentation, and receipts to identify quantity and price discrepancies</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/syncrofy_supply_chain/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-1053 size-thumbnail"
                    src="../../SupplyChain_6_PartnerCollab-300x300.png"
                    alt="Partner Collaboration"
                    width="75"
                    height="75"
                    srcSet="
                      ../../SupplyChain_6_PartnerCollab-300x300.png 300w,
                      ../../SupplyChain_6_PartnerCollab.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I need to connect, collaborate, and solve discrepancies across departments and locations, and with suppliers and customers</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/integration/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="wp-image-648 size-thumbnail aligncenter"
                    src="../../SupplyChain_1_ExecIssues-300x300.png"
                    alt="Execution Issues"
                    width="75"
                    height="75"
                    srcSet="../../SupplyChain_1_ExecIssues-300x300.png 300w, ../../SupplyChain_1_ExecIssues.png 800w"
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I’m new to IBM Sterling B2B/MFT and need guidance from a partner</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>
        </div>
        <div className="row justify-content-center">
          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/integration/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-1054 size-thumbnail"
                    src="../../SupplyChain_7_Monitoring-300x300.png"
                    alt="Effective Monitoring"
                    width="75"
                    height="75"
                    srcSet="../../SupplyChain_7_Monitoring-300x300.png 300w, ../../SupplyChain_7_Monitoring.png 800w"
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I’m lacking effective monitoring &amp; reporting on my supply chain</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/integration/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-1055 size-thumbnail"
                    src="../../SupplyChain_8_TransactionVisibility-300x300.png"
                    alt="Transaction Visibility"
                    width="75"
                    height="75"
                    srcSet="
                      ../../SupplyChain_8_TransactionVisibility-300x300.png 300w,
                      ../../SupplyChain_8_TransactionVisibility.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I need greater visibility into my transactions</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/integration/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-1056 size-thumbnail"
                    src="../../SupplyChain_9_SpinningWheels-300x300.png"
                    alt="Spinning Wheels"
                    width="75"
                    height="75"
                    srcSet="
                      ../../SupplyChain_9_SpinningWheels-300x300.png 300w,
                      ../../SupplyChain_9_SpinningWheels.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I’m spinning my wheels on tasks that require manual effort</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>
        </div>
        <div className="row justify-content-center">
          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/integration/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="wp-image-657 size-thumbnail aligncenter"
                    src="../../SupplyChain_10_ComplexIntegrations-300x300.png"
                    alt="Complex Integrations"
                    width="75"
                    height="75"
                    srcSet="
                      ../../SupplyChain_10_ComplexIntegrations-300x300.png 300w,
                      ../../SupplyChain_10_ComplexIntegrations.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>I’m struggling to integrate complex B2Bi/EDI processes</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/integration/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="wp-image-658 size-thumbnail aligncenter"
                    src="../../SupplyChain_11_Security-300x300.png"
                    alt="Security"
                    width="75"
                    height="75"
                    srcSet="../../SupplyChain_11_Security-300x300.png 300w, ../../SupplyChain_11_Security.png 800w"
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>My transactions &amp; communications are not secure</h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>

    <div className="content-container-lg bg-cover bg-shapes-1">
      <div className="container">
        <div className="row justify-content-center">
          <div className="col-md-8">
            <h2 style={{textAlign: "center"}}>Interested in learning more?</h2>
            <p style={{textAlign: "center"}}><a className="btn btn-aqua" href="https://info.coenterprise.com/consultation">Request a Consultation</a></p>
          </div>
        </div>
      </div>
    </div>
    </Layout>)
}

export default SupplyChain;