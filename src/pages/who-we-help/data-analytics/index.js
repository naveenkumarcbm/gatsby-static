import React from "react";
import Layout from "../../../components/layout";

const DataAnalytics = () => {
  return (<Layout headerclassName="white-bg-header">
    <div className="linear-gradient-1">
      <div className="aspot-bg-1 bg-cover bg-cover-bottom">
        <div className="content-container d-flex align-items-center">
          <div className="container">
            <div className="row justify-content-center">
              <div className="col-md-6 text-center">
                <p style={{textAlign: "center"}}>WHO WE HELP</p>
                <h1 className="heading-underline underline-blue" style={{textAlign: "center"}}>
                  Data Analytics &amp; Business Intelligence
                </h1>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="content-container">
        <div className="container">
          <div className="row">
            <div className="col-md-6 content-section">
              <h2 className="h3">
                Integrate and transform your data while discovering valuable information and insights. Draw meaningful conclusions and empower data-driven decision-making across your organization.
              </h2>
            </div>
            <div className="col-md">
              <p>
                Having access to all your data under a single platform was once a dream—but now it’s a reality. Our dedicated analytics practice gives you the tools you need to gain valuable insights,
                answers complex business questions, make better-informed decisions, and ultimately take back control over your data—from people you can trust.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div className="content-container bg-full-width bg-shapes-4">
      <div className="container">
        <div className="row justify-content-center">
          <div className="col-md-6 content-section">
            <h2 className="heading-underline underline-purple" style={{textAlign: "center"}}>
              How Can We Help You?
            </h2>
          </div>
        </div>

        <div className="row justify-content-center">
          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/data-analytics/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-717 size-thumbnail"
                    src="../../DataAnalyticsAndBusiness_1_DataVisibility-300x300.png"
                    alt="Lacking Data Visibility"
                    width="75"
                    height="75"
                    srcSet="
                      ../../DataAnalyticsAndBusiness_1_DataVisibility-300x300.png 300w,
                      ../../DataAnalyticsAndBusiness_1_DataVisibility.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>
                  I’m lacking sufficient visibility into my data
                </h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/data-analytics/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-718 size-thumbnail"
                    src="../../DataAnalyticsAndBusiness_2_NewVizualizations-300x300.png"
                    alt="New Data Visualizations"
                    width="75"
                    height="75"
                    srcSet="
                      ../../DataAnalyticsAndBusiness_2_NewVizualizations-300x300.png 300w,
                      ../../DataAnalyticsAndBusiness_2_NewVizualizations.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>
                  I’m looking for new ways to visualize my data
                </h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/data-analytics/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-719 size-thumbnail"
                    src="../../DataAnalyticsAndBusiness_1_RampUp-300x300.png"
                    alt="Tableau Ramp Up"
                    width="75"
                    height="75"
                    srcSet="
                      ../../DataAnalyticsAndBusiness_1_RampUp-300x300.png 300w,
                      ../../DataAnalyticsAndBusiness_1_RampUp.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>
                  I need to ramp up Tableau user adoption across my organization
                </h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>
        </div>
        <div className="row justify-content-center">
          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/data-analytics/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-720 size-thumbnail"
                    src="../../DataAnalyticsAndBusiness_3_DataMerge-300x300.png"
                    alt="Data Merge"
                    width="75"
                    height="75"
                    srcSet="
                      ../../DataAnalyticsAndBusiness_3_DataMerge-300x300.png 300w,
                      ../../DataAnalyticsAndBusiness_3_DataMerge.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>
                  I have data in several systems and I need help integrating it into one for easy data analysis
                </h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/data-analytics/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-721 size-thumbnail"
                    src="../../DataAnalyticsAndBusiness_5_HealthCheck-300x300.png"
                    alt="Tableau Servers Health Check"
                    width="75"
                    height="75"
                    srcSet="
                      ../../DataAnalyticsAndBusiness_5_HealthCheck-300x300.png 300w,
                      ../../DataAnalyticsAndBusiness_5_HealthCheck.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>
                  My Tableau servers need a health check
                </h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/data-analytics/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-722 size-thumbnail"
                    src="../../DataAnalyticsAndBusiness_6_ActionableBlueprint-300x300.png"
                    alt="Actionable Tableau Blueprint"
                    width="75"
                    height="75"
                    srcSet="
                      ../../DataAnalyticsAndBusiness_6_ActionableBlueprint-300x300.png 300w,
                      ../../DataAnalyticsAndBusiness_6_ActionableBlueprint.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>
                  I’m struggling to make Tableau Blueprint actionable
                </h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>
        </div>
        <div className="row justify-content-center">
          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/data-analytics/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-723 size-thumbnail"
                    src="../../DataAnalyticsAndBusiness_7_NewToTableau-300x300.png"
                    alt="New Tableau User"
                    width="75"
                    height="75"
                    srcSet="
                      ../../DataAnalyticsAndBusiness_7_NewToTableau-300x300.png 300w,
                      ../../DataAnalyticsAndBusiness_7_NewToTableau.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>
                  I’m new to Tableau and need a partner to show me the ropes
                </h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/data-analytics/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-724 size-thumbnail"
                    src="../../DataAnalyticsAndBusiness_8_IntegrateTableau-300x300.png"
                    alt="Integrate Tableau"
                    width="75"
                    height="75"
                    srcSet="
                      ../../DataAnalyticsAndBusiness_8_IntegrateTableau-300x300.png 300w,
                      ../../DataAnalyticsAndBusiness_8_IntegrateTableau.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>
                  I need to integrate Tableau into Salesforce
                </h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>

          <div className="col-lg-4 content-section">
            <a className="text-center hover-bucket h-100" href="/solutions/data-analytics/">
              <div className="hover-bucket-inner box-shadow-1 box-shadow-padding-2 h-100">
                <p>
                  <img
                    loading="lazy"
                    className="aligncenter wp-image-725 size-thumbnail"
                    src="../../DataAnalyticsAndBusiness_9_TableauTraining-300x300.png"
                    alt="Tableau Training"
                    width="75"
                    height="75"
                    srcSet="
                      ../../DataAnalyticsAndBusiness_9_TableauTraining-300x300.png 300w,
                      ../../DataAnalyticsAndBusiness_9_TableauTraining.png         800w
                    "
                    sizes="(max-width: 75px) 100vw, 75px"
                  />
                </p>
                <h3 className="h4" style={{textAlign: "center"}}>
                  My team is in need of effective Tableau training
                </h3>
                <p className="learn-more">Learn More</p>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>

    <div className="content-container-lg bg-cover bg-shapes-1">
      <div className="container">
        <div className="row justify-content-center">
          <div className="col-md-8">
            <h2 style={{textAlign: "center"}}>Interested in learning more?</h2>
            <p style={{textAlign: "center"}}>
              <a className="btn btn-aqua" href="https://info.coenterprise.com/consultation">
                Request a Consultation
              </a>
            </p>
          </div>
        </div>
      </div>
    </div>
  </Layout>);
};

export default DataAnalytics;
