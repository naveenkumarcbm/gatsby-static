import React from 'react'
import Layout from '../../components/layout';

const Resources = () => {
  return (
    <Layout headerclassName="white-bg-header">
      <div className="aspot-bg-1 bg-cover bg-cover-bottom">
      <div className="content-container d-flex align-items-center">
        <div className="container">
          <div className="row justify-content-center">
            <div className="col-md-6 text-center">
              <h1 className="heading-underline underline-blue" style={{textAlign: "center"}}>Resources</h1>
              <h2 className="paragraph" style={{textAlign: "center"}}>Connect. Collaborate. Trade. Learn how you can increase productivity and drive better decisions.</h2>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div className="content-container">
      <div className="container">
        <div className="row">
          <div className="col-lg-4 content-section-tight">
            <div className="resource-bucket-wrapper h-100 content-tight">
              <div
                className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                style={{backgroundImage: "url(/uploads/2021/02/Industry-Today-1-300x110.png)"}}
              >
                <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-2 h-100 news">
                  <div className="resource-bucket-top">
                    <p className="post-type scrunch-top">News</p>
                    <h4 className="font-weight-bold">
                      <a href="/news/industry-today-speed-and-precision-the-supply-chain-differentiators/"
                        >Speed and Precision: The Supply Chain Differentiators | Industry Today</a
                      >
                    </h4>
                  </div>
                  <div className="resource-bucket-bottom">
                    <p className="scrunch-bottom"><a href="/news/industry-today-speed-and-precision-the-supply-chain-differentiators/" className="arrow-link">Read Now</a></p>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div className="col-lg-4 content-section-tight">
            <div className="resource-bucket-wrapper h-100 content-tight">
              <div
                className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                style={{backgroundImage: "url(/uploads/2021/01/chargebacks_BlogPost_feauredimage_02-300x158.png)"}}
              >
                <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-2 h-100 blog">
                  <div className="resource-bucket-top">
                    <p className="post-type scrunch-top">Blog</p>
                    <h4 className="font-weight-bold">
                      <a href="/blog/free-ebook-top-5-actions-to-take-to-reduce-chargebacks-in-2021/">Free eBook: Top 5 Actions to Take to Reduce Chargebacks in 2021</a>
                    </h4>
                    <p><small>By: Pasquale Gatti</small></p>
                  </div>
                  <div className="resource-bucket-bottom">
                    <p className="scrunch-bottom"><a href="/blog/free-ebook-top-5-actions-to-take-to-reduce-chargebacks-in-2021/" className="arrow-link">Read Now</a></p>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div className="col-lg-4 content-section-tight">
            <div className="resource-bucket-wrapper h-100 content-tight">
              <div className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse" style={{backgroundImage: "url(/uploads/2021/02/SDC-logo.png)"}}>
                <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-2 h-100 news">
                  <div className="resource-bucket-top">
                    <p className="post-type scrunch-top">News</p>
                    <h4 className="font-weight-bold">
                      <a href="/news/chargebacks-the-hidden-costs-lurking-in-the-supply-chain/"
                        >Chargebacks: The Hidden Costs Lurking in the Supply Chain | Supply &amp; Demand Chain Executive</a
                      >
                    </h4>
                  </div>
                  <div className="resource-bucket-bottom">
                    <p className="scrunch-bottom"><a href="/news/chargebacks-the-hidden-costs-lurking-in-the-supply-chain/" className="arrow-link">Read Now</a></p>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div className="col-lg-4 content-section-tight">
            <div className="resource-bucket-wrapper h-100 content-tight">
              <div
                className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                style={{backgroundImage: "url(/uploads/2020/12/SyncrofyIT_BlogPost_02-300x158.png)"}}
              >
                <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-2 h-100 blog">
                  <div className="resource-bucket-top">
                    <p className="post-type scrunch-top">Blog</p>
                    <h4 className="font-weight-bold">
                      <a href="/blog/how-the-right-tool-can-ensure-a-seamless-and-successful-enterprise-wide-it-migration/"
                        >How the Right Tool Can Ensure a Seamless and Successful Enterprise-Wide IT Migration</a
                      >
                    </h4>
                    <p><small>By: Jennifer Tattenbaum</small></p>
                  </div>
                  <div className="resource-bucket-bottom">
                    <p className="scrunch-bottom">
                      <a href="/blog/how-the-right-tool-can-ensure-a-seamless-and-successful-enterprise-wide-it-migration/" className="arrow-link">Read Now</a>
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div className="col-lg-4 content-section-tight">
            <div className="resource-bucket-wrapper h-100 content-tight">
              <div
                className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                style={{backgroundImage: "url(/uploads/2021/02/supply-chain-brain-logo-300x50.gif)"}}
              >
                <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-2 h-100 news">
                  <div className="resource-bucket-top">
                    <p className="post-type scrunch-top">News</p>
                    <h4 className="font-weight-bold">
                      <a href="/news/taking-charge-against-chargebacks-in-the-supply-chain/">Taking Charge Against Chargebacks in the Supply Chain | Supply Chain Brain</a>
                    </h4>
                  </div>
                  <div className="resource-bucket-bottom">
                    <p className="scrunch-bottom"><a href="/news/taking-charge-against-chargebacks-in-the-supply-chain/" className="arrow-link">Read Now</a></p>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div className="col-lg-4 content-section-tight">
            <div className="resource-bucket-wrapper h-100 content-tight">
              <div className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse" style={{backgroundImage: "url()"}}>
                <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-2 h-100 in the press">
                  <div className="resource-bucket-top">
                    <p className="post-type scrunch-top">In The Press</p>
                    <h4 className="font-weight-bold">
                      <a
                        target="_blank"
                        href="https://www.prweb.com/releases/coenterprise_releases_new_supply_chain_solution_to_help_companies_achieve_full_visibility_eliminate_chargebacks_prevent_late_payments_shipments_and_minimize_disruption/prweb17285266.htm"
                        >CoEnterprise Releases New Cloud-Based Supply Chain Solution</a
                      >
                    </h4>
                  </div>
                  <div className="resource-bucket-bottom">
                    <p className="scrunch-bottom">
                      <a
                        target="_blank"
                        href="https://www.prweb.com/releases/coenterprise_releases_new_supply_chain_solution_to_help_companies_achieve_full_visibility_eliminate_chargebacks_prevent_late_payments_shipments_and_minimize_disruption/prweb17285266.htm"
                        className="arrow-link"
                        >Read Now</a
                      >
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div className="content-container pb-0">
      <div className="resources-nav">
        <div className="container">
          <ul className="resources-nav-menu">
            <li className="active">
              <a href="/resources/">
                <i className="fas fa-stars"></i>
                Featured
              </a>
            </li>
            <li>
              <a href="/blog/">
                <i className="fas fa-file-alt"></i>
                Blog
              </a>
            </li>
            <li>
              <a href="/webinars/">
                <i className="far fa-microphone-alt"></i>
                Webinars
              </a>
            </li>
            <li>
              <a href="/in-the-press/">
                <i className="fas fa-comment-plus"></i>
                In The Press
              </a>
            </li>
            <li>
              <a href="/events/">
                <i className="fas fa-calendar-day"></i>
                Events
              </a>
            </li>
            <li>
              <a href="/guides-reports/">
                <i className="fas fa-file-chart-line"></i>
                Guides &amp; Reports
              </a>
            </li>
            <li>
              <a href="/product-collateral/">
                <i className="fas fa-book-open"></i>
                Product Collateral
              </a>
            </li>
          </ul>
        </div>
      </div>
    </div>

    <div className="content-container">
      <div className="container">
        <div className="content-section">
          <div className="row justify-content-between align-items-center content-section-tight">
            <div className="col-md">
              <h3 className="h4 scrunch font-weight-bold">Featured Blogs</h3>
            </div>
            <div className="col-md text-md-right">
              <a href="/blog/" className="arrow-link green-link">See all</a>
            </div>
          </div>
          <div className="row">
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/04/snowflakeIII_BlogPost_FeaturedImage-300x169.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay1">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Blog</p>
                      <h6 className="font-weight-bold">
                        <a href="/blog/mastering-the-basics-with-snowflake-rbac-part-iii-padlocking-the-schema/"
                          >Mastering the Basics with Snowflake RBAC, Part III: Padlocking the Schema</a
                        >
                      </h6>
                      <p><small>By: Adam Nathan</small></p>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a href="/blog/mastering-the-basics-with-snowflake-rbac-part-iii-padlocking-the-schema/" className="arrow-link text-small">Read Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/04/RecievingOperations_BlogPost_01-300x169.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay2">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Blog</p>
                      <h6 className="font-weight-bold">
                        <a href="/blog/3-easy-ways-to-improve-your-receiving-operations/">3 Easy Ways to Improve Your Receiving Operations</a>
                      </h6>
                      <p><small>By: Jennifer Tattenbaum</small></p>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom"><a href="/blog/3-easy-ways-to-improve-your-receiving-operations/" className="arrow-link text-small">Read Now</a></p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/04/TableauPrep_Blog_header-300x158.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay3">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Blog</p>
                      <h6 className="font-weight-bold">
                        <a href="/blog/how-to-know-if-tableau-prep-is-a-good-fit-for-your-business/">How to Know if Tableau Prep is a Good Fit for Your Business</a>
                      </h6>
                      <p><small>By: Kayla Grieme</small></p>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom"><a href="/blog/how-to-know-if-tableau-prep-is-a-good-fit-for-your-business/" className="arrow-link text-small">Read Now</a></p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/04/snowflakeII-300x158.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay4">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Blog</p>
                      <h6 className="font-weight-bold">
                        <a href="/blog/mastering-the-basics-with-snowflake-rbac-part-ii-delegating-down/"
                          >Mastering the Basics With Snowflake RBAC, Part II: Delegating Down</a
                        >
                      </h6>
                      <p><small>By: Adam Nathan</small></p>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a href="/blog/mastering-the-basics-with-snowflake-rbac-part-ii-delegating-down/" className="arrow-link text-small">Read Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className="content-section">
          <div className="row justify-content-between align-items-center content-section-tight">
            <div className="col-md">
              <h3 className="h4 scrunch font-weight-bold">Featured Webinars</h3>
            </div>
            <div className="col-md text-md-right">
              <a href="/webinars/" className="arrow-link green-link">See all</a>
            </div>
          </div>
          <div className="row">
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2020/04/b2bassurancepreview-300x300.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay5">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Webinar</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/ondemand-webinar-data-management-with-tableau-prep">Taking Control of Your Data Management with Tableau Prep</a>
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a target="_blank" href="https://info.coenterprise.com/ondemand-webinar-data-management-with-tableau-prep" className="arrow-link text-small">Watch Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2020/09/BlogPost_Fines-Chargebacks-Slowing-Down-Your-Supply-Chain_02-300x158.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay6">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Webinar</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/supply-chain-2020-in-review-ondemand-webinar">A Look Back at 2020 Trends and What Retailers Can Expect in 2021</a>
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom"><a target="_blank" href="https://info.coenterprise.com/supply-chain-2020-in-review-ondemand-webinar" className="arrow-link text-small">Watch Now</a></p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2020/12/SyncrofyIT_BlogPost_02-300x158.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay7">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Webinar</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/driving-2020-holiday-insights-with-data-on-demand">Driving 2020 Holiday Insights with Data</a>
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a target="_blank" href="https://info.coenterprise.com/driving-2020-holiday-insights-with-data-on-demand" className="arrow-link text-small">Watch Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2020/07/CoEnterprise_Supply_Chain_Predictions-300x133.jpg)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay1">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Webinar</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/nightmare-before-christmas-ondemand"
                          >How to Achieve Effective Supply Chain Visibility to Avoid the Nightmare Before Christmas this Holiday Season</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom"><a target="_blank" href="https://info.coenterprise.com/nightmare-before-christmas-ondemand" className="arrow-link text-small">Watch Now</a></p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className="content-section">
          <div className="row justify-content-between align-items-center content-section-tight">
            <div className="col-md">
              <h3 className="h4 scrunch font-weight-bold">Featured News</h3>
            </div>
            <div className="col-md text-md-right">
              <a href="/news/" className="arrow-link green-link">See all</a>
            </div>
          </div>
          <div className="row">
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/05/supply-chain-software-technology-1000x600-1-300x180.jpg)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay2">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">News</p>
                      <h6 className="font-weight-bold">
                        <a href="/news/coenterprise-upgrades-let-firms-fix-supply-chain-challenges/">CoEnterprise Upgrades Let Firms Fix Supply Chain Challenges</a>
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom"><a href="/news/coenterprise-upgrades-let-firms-fix-supply-chain-challenges/" className="arrow-link text-small">Read Now</a></p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse" style={{backgroundImage: "url(/uploads/2021/03/Food-Logistics-2.png)"}}>
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay3">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">News</p>
                      <h6 className="font-weight-bold">
                        <a href="/news/the-100-day-supply-chain-review-part-2-supply-chain-visibility-food-logistics/"
                          >The 100-Day Supply Chain Review, Part 2: Supply Chain Visibility | Food Logistics</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a href="/news/the-100-day-supply-chain-review-part-2-supply-chain-visibility-food-logistics/" className="arrow-link text-small">Read Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse" style={{backgroundImage: "url(/uploads/2021/03/Food-Logistics-2.png)"}}>
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay4">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">News</p>
                      <h6 className="font-weight-bold">
                        <a href="/news/the-100-day-supply-chain-review-part-1-addressing-risks-food-logistics/"
                          >The 100-Day Supply Chain Review, Part 1: Addressing Risks | Food Logistics</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a href="/news/the-100-day-supply-chain-review-part-1-addressing-risks-food-logistics/" className="arrow-link text-small">Read Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/03/Retail-Hospitality-Hub-300x34.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay5">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">News</p>
                      <h6 className="font-weight-bold">
                        <a href="/news/to-achieve-supply-chain-visibility-get-ready-to-digitize-retail-hospitality-hub/"
                          >To Achieve Supply Chain Visibility, Get Ready to Digitize | Retail &amp; Hospitality Hub</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a href="/news/to-achieve-supply-chain-visibility-get-ready-to-digitize-retail-hospitality-hub/" className="arrow-link text-small">Read Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className="content-section">
          <div className="row justify-content-between align-items-center content-section-tight">
            <div className="col-md">
              <h3 className="h4 scrunch font-weight-bold">Featured In The Press</h3>
            </div>
            <div className="col-md text-md-right">
              <a href="/in-the-press/" className="arrow-link green-link">See all</a>
            </div>
          </div>
          <div className="row">
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/03/MaheshN_Headshot-300x300.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay6">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">In The Press</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://www.businesswire.com/news/home/20210316005179/en/Mahesh-Nandyala-Joins-CoEnterprise-as-Chief-Product-Officer"
                          >Mahesh Nandyala Joins CoEnterprise as Chief Product Officer</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a target="_blank" href="https://www.businesswire.com/news/home/20210316005179/en/Mahesh-Nandyala-Joins-CoEnterprise-as-Chief-Product-Officer" className="arrow-link text-small"
                          >Read Now</a
                        >
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse" style={{backgroundImage: "url()"}}>
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay7">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">In The Press</p>
                      <h6 className="font-weight-bold">
                        <a
                          target="_blank"
                          href="https://www.prweb.com/releases/coenterprise_announces_new_vice_president_of_professional_services_to_refine_processes_and_fuel_continued_growth/prweb17596294.htm"
                          >CoEnterprise Announces New Vice President of Professional Services to Refine Processes and Fuel Continued Growth</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a
                          target="_blank"
                          href="https://www.prweb.com/releases/coenterprise_announces_new_vice_president_of_professional_services_to_refine_processes_and_fuel_continued_growth/prweb17596294.htm"
                          className="arrow-link text-small"
                          >Read Now</a
                        >
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse" style={{backgroundImage: "url()"}}>
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay1">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">In The Press</p>
                      <h6 className="font-weight-bold">
                        <a
                          target="_blank"
                          href="https://www.prweb.com/releases/coenterprise_releases_new_supply_chain_solution_to_help_companies_achieve_full_visibility_eliminate_chargebacks_prevent_late_payments_shipments_and_minimize_disruption/prweb17285266.htm"
                          >CoEnterprise Releases New Cloud-Based Supply Chain Solution</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a
                          target="_blank"
                          href="https://www.prweb.com/releases/coenterprise_releases_new_supply_chain_solution_to_help_companies_achieve_full_visibility_eliminate_chargebacks_prevent_late_payments_shipments_and_minimize_disruption/prweb17285266.htm"
                          className="arrow-link text-small"
                          >Read Now</a
                        >
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2020/04/Mark-Jennifer-PR-300x161.jpg)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay2">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">In The Press</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://markets.businessinsider.com/news/stocks/coenterprise-strengthens-executive-team-with-two-new-additions-1028703911"
                          >CoEnterprise Strengthens Executive Team with Two New Additions</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a
                          target="_blank"
                          href="https://markets.businessinsider.com/news/stocks/coenterprise-strengthens-executive-team-with-two-new-additions-1028703911"
                          className="arrow-link text-small"
                          >Read Now</a
                        >
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className="content-section">
          <div className="row justify-content-between align-items-center content-section-tight">
            <div className="col-md">
              <h3 className="h4 scrunch font-weight-bold">Featured Guides &amp; Reports</h3>
            </div>
            <div className="col-md text-md-right">
              <a href="/guides-reports/" className="arrow-link green-link">See all</a>
            </div>
          </div>
          <div className="row">
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/05/Analytics_Snowflake_Content-300x209.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay3">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Guides &amp; Reports</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/ebook-tableau-prep">eBook: How Tableau Prep Can Save You Time &amp; Make Your Life Easier as a Data Analyst</a>
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom"><a target="_blank" href="https://info.coenterprise.com/ebook-tableau-prep" className="arrow-link text-small">Read Now</a></p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/03/vendorcompliance_ebook-300x158.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay4">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Guides &amp; Reports</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/hubfs/eBooks/CoEnterprise_5_Steps_for_Effective_Retail_Vendor_Compliance_Program.pdf"
                          >eBook: 5 Steps for an Effective Retail Vendor Compliance Program</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a target="_blank" href="https://info.coenterprise.com/hubfs/eBooks/CoEnterprise_5_Steps_for_Effective_Retail_Vendor_Compliance_Program.pdf" className="arrow-link text-small"
                          >Read Now</a
                        >
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/02/chargebacks_ebook_featuredimage-300x157.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay5">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Guides &amp; Reports</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/ebook-vendor-chargebacks">eBook: Top 5 Actions to Take to Reduce Chargebacks in Your Vendor Compliance Program</a>
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom"><a target="_blank" href="https://info.coenterprise.com/ebook-vendor-chargebacks" className="arrow-link text-small">Read Now</a></p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/04/TableauPrep_Blog_header-300x158.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay6">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Guides &amp; Reports</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/ebook_benefits_of_automating_file_transfer_migration"
                          >eBook: Why You Need to Automate Your File Transfer Migration Now</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a target="_blank" href="https://info.coenterprise.com/ebook_benefits_of_automating_file_transfer_migration" className="arrow-link text-small">Read Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className="content-section">
          <div className="row justify-content-between align-items-center content-section-tight">
            <div className="col-md">
              <h3 className="h4 scrunch font-weight-bold">Featured Product Collateral</h3>
            </div>
            <div className="col-md text-md-right">
              <a href="/product-collateral/" className="arrow-link green-link">See all</a>
            </div>
          </div>
          <div className="row">
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/05/snowflake_overview_featuredimage-300x158.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay7">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Product Collateral</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/hubfs/Collateral/Analytics/CoEnterprise_Analytics_Snowflake_Overview.pdf">Snowflake Overview</a>
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a target="_blank" href="https://info.coenterprise.com/hubfs/Collateral/Analytics/CoEnterprise_Analytics_Snowflake_Overview.pdf" className="arrow-link text-small">Read Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2020/03/Syncrofy-cloud-300x165.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay1">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Product Collateral</p>
                      <h6 className="font-weight-bold"><a target="_blank" href="https://info.coenterprise.com/hubfs/COE_ASN%20Compliance%201%20pager.pdf">Syncrofy’s ASN Compliance Solution</a></h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom"><a target="_blank" href="https://info.coenterprise.com/hubfs/COE_ASN%20Compliance%201%20pager.pdf" className="arrow-link text-small">Read Now</a></p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/03/Syncrofy-for-EDI-vs-Syncrofy-for-Supply-Chain-300x173.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay2">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Product Collateral</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/hubfs/Syncrofy%20-%20Syncrofy%20EDI%20SC%20Comparison%20Document.pdf"
                          >Syncrofy for EDI &amp; IT vs Syncrofy for Supply Chain: Choosing the Solution That is Right For You</a
                        >
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a target="_blank" href="https://info.coenterprise.com/hubfs/Syncrofy%20-%20Syncrofy%20EDI%20SC%20Comparison%20Document.pdf" className="arrow-link text-small">Read Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-md-6 col-lg-3 content-section-tight">
              <div className="resource-bucket-wrapper h-100 content-tight">
                <div
                  className="resource-bucket box-shadow-1 h-100 bg-cover content-inverse links-inverse"
                  style={{backgroundImage: "url(/uploads/2021/03/professionalservicesoverview-300x158.png)"}}
                >
                  <div className="resource-bucket-spacing resource-bucket-color-overlay box-shadow-padding-4 h-100 overlay3">
                    <div className="resource-bucket-top">
                      <p className="post-type scrunch-top">Product Collateral</p>
                      <h6 className="font-weight-bold">
                        <a target="_blank" href="https://info.coenterprise.com/hubfs/Collateral/Integration/CoEnterprise_Professional_Services_Overview.pdf">Professional Services Overview</a>
                      </h6>
                    </div>
                    <div className="resource-bucket-bottom">
                      <p className="scrunch-bottom">
                        <a target="_blank" href="https://info.coenterprise.com/hubfs/Collateral/Integration/CoEnterprise_Professional_Services_Overview.pdf" className="arrow-link text-small">Read Now</a>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div className="content-container-lg bg-cover bg-shapes-1">
      <div className="container">
        <div className="row justify-content-center">
          <div className="col-md-10 text-center">
            <h2 style={{textAlign: "center"}}>Subscribe to our resources!</h2>
            <p style={{textAlign: "center"}}>Sign up to receive our latest eBooks, webinars, blog posts, newsletter, event invitations, and much more.</p>
          </div>
        </div>
        <div className="row justify-content-center">
          <div className="col-10 col-md-8 col-lg-6 col-xl-4 text-center">
            <div className="hubspot-form">
              <div className="hbspt-form" id="hbspt-form-1622091775017-9170263085">
                <form
                  novalidate=""
                  accept-charset="UTF-8"
                  action="https://forms.hsforms.com/submissions/v3/public/submit/formsnext/multipart/3283290/2ff8fdfc-3688-4d3b-805a-cb0dd402deea"
                  enctype="multipart/form-data"
                  id="hsForm_2ff8fdfc-3688-4d3b-805a-cb0dd402deea"
                  method="POST"
                  className="
                    hs-form
                    stacked
                    hs-form-private
                    hsForm_2ff8fdfc-3688-4d3b-805a-cb0dd402deea
                    hs-form-2ff8fdfc-3688-4d3b-805a-cb0dd402deea hs-form-2ff8fdfc-3688-4d3b-805a-cb0dd402deea_6b6f4891-7519-4b09-8157-9a5d737cf39e
                  "
                  data-form-id="2ff8fdfc-3688-4d3b-805a-cb0dd402deea"
                  data-portal-id="3283290"
                  target="target_iframe_2ff8fdfc-3688-4d3b-805a-cb0dd402deea"
                  data-reactid=".hbspt-forms-0"
                >
                  <div className="hs_email hs-email hs-fieldtype-text field hs-form-field" data-reactid=".hbspt-forms-0.1:$0">
                    <label
                      id="label-email-2ff8fdfc-3688-4d3b-805a-cb0dd402deea"
                      className=""
                      placeholder="Enter your Email"
                      for="email-2ff8fdfc-3688-4d3b-805a-cb0dd402deea"
                      data-reactid=".hbspt-forms-0.1:$0.0"
                      ><span data-reactid=".hbspt-forms-0.1:$0.0.0">Email</span><span className="hs-form-required" data-reactid=".hbspt-forms-0.1:$0.0.1">*</span></label
                    >
                    <legend className="hs-field-desc" style={{display: 'none'}} data-reactid=".hbspt-forms-0.1:$0.1"></legend>
                    <div className="input" data-reactid=".hbspt-forms-0.1:$0.$email">
                      <input
                        id="email-2ff8fdfc-3688-4d3b-805a-cb0dd402deea"
                        className="hs-input"
                        type="email"
                        name="email"
                        required=""
                        placeholder=""
                        defaultValue=""
                        autoComplete="email"
                        data-reactid=".hbspt-forms-0.1:$0.$email.0"
                        inputmode="email"
                      />
                    </div>
                  </div>
                  <noscript data-reactid=".hbspt-forms-0.2"></noscript>
                  <div className="hs_submit hs-submit" data-reactid=".hbspt-forms-0.5">
                    <div className="hs-field-desc" style={{display: 'none'}} data-reactid=".hbspt-forms-0.5.0"></div>
                    <div className="actions" data-reactid=".hbspt-forms-0.5.1"><input type="submit" value="Subscribe!" className="hs-button primary large" data-reactid=".hbspt-forms-0.5.1.0" /></div>
                  </div>
                  <noscript data-reactid=".hbspt-forms-0.6"></noscript
                  ><input
                    name="hs_context"
                    type="hidden"
                    defaultValue='{"rumScriptExecuteTime":698.3250000048429,"rumServiceResponseTime":1017.9199999547563,"rumFormRenderTime":1.5799999819137156,"rumTotalRenderTime":1021.2900000042282,"rumTotalRequestTime":318.31500004045665,"lang":"en","renderRawHtml":"true","embedAtTimestamp":"1622091775209","formDefinitionUpdatedAt":"1507920873834","pageUrl":"/resources/","pageTitle":"Resources | CoEnterprise","source":"FormsNext-static","timestamp":1622091775209,"userAgent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36","referrer":"/about/","originalEmbedContext":{"portalId":"3283290","formId":"2ff8fdfc-3688-4d3b-805a-cb0dd402deea","target":"#hbspt-form-1622091775017-9170263085","hutk":"6434b89863edbbd997df799fd29ea9a7"},"formTarget":"#hbspt-form-1622091775017-9170263085","correlationId":"349ee057-9d4d-4d57-b1a8-9082d13fbd0d","hutk":"6434b89863edbbd997df799fd29ea9a7","useRecaptchaEnterprise":true}'
                    data-reactid=".hbspt-forms-0.7"
                  /><iframe
                    name="target_iframe_2ff8fdfc-3688-4d3b-805a-cb0dd402deea"
                    style={{display: 'none'}}
                    data-reactid=".hbspt-forms-0.8"
                    src="./saved_resource.html"
                  ></iframe>
                </form>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    </Layout>
  )
}

export default Resources;